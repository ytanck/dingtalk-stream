...
class CardReplier {
  ...
  async createAndDeliverCard(
    ...
    try {
      ...
      console.error("craete and deliver card error:create ");
  ...
  async putCardData(options: PutCardDataOptions): Promise<void> {
    ...
    try {
      ...
      console.error("update card error: putdata");
      // console.error("update card error: ", error);
    }
    return;
  }
  async streanUpdateCard(options: PutContentOptions): Promise<void>{
    const accessToken = await this.dingtalkClient.getAccessToken();
    if (!accessToken) {
      console.error(
        "streanUpdateCard:CardResponder.sendCard failed, cannot get dingtalk access token"
      );
      return;
    }
    const { cardInstanceId, ...restOptions } = options;
    const data: any = {
      outTrackId: cardInstanceId,
      ...restOptions,
    };
    const url = DINGTALK_OPENAPI_ENDPOINT + "/v1.0/card/streaming";
    try {
      const response = await axios.put(url, data, {
        headers: CardReplier.getRequestHeader(accessToken),
      });
      ...
      console.error("update streamCard error: ", error);
    }
    return;
  }
}
